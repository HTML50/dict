...
chrome.runtime.getBackgroundPage(function(w) {
  ...
  ctrlSearchStatus.disabled = isDragSearch ? false : true;
  if (!isDragSearch) {
...
searchHotkey.addEventListener('click', function(e) {
  e.preventDefault();
  chrome.tabs.create({
    url: 'chrome://extensions/configureCommands',
    active: true
...
gloablSearchHotkey.addEventListener('click', function(e) {
  e.preventDefault();
  chrome.tabs.create({
    url: 'chrome://extensions/configureCommands',
    active: true
...
dragAndSearchStatus.addEventListener('click', function() {
  ...
  chrome.runtime.sendMessage({ setDragSearch: isDragSearch });

  //send isDragSearch to all tabs
  chrome.tabs.query({}, function(tabs) {
    for (var i = 0; i < tabs.length; i++) {
      chrome.tabs.sendMessage(tabs[i].id, { dragSearchStatusChange: isDragSearch });
    }
  });
})
...
document.addEventListener('change', function(e) {
  ...
  setTimeout(function() { statusTip.style.opacity = 0 }, 3000)
})
